...
Fmatrix = readtable('TMM Spacecraftv2.1.xlsx','Sheet','F');
np.F = table2array(Fmatrix(:,2:end)); %view factor from ith node to jth node, symmetric n*n matrix
...
disp('The steady state temperature (K) of the components are')

for i = 1: length(np.name)
    disp(np.name{i})
    disp(T(i))
end
